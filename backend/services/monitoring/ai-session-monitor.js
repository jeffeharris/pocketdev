...
export class AISessionMonitor {
  ...
  registerPatterns() {
    ...
    const patterns = {
      ...
      "ai-yes-no": {
        pattern:
          /(?:Should I|Would you like me to|Shall I|Can I|May I).*\?\s*\(y\/n\)\s*:?\s*$/i,
        type: "input",
        priority: "high",
        handler: (match, sessionId) => ({
          state: "waiting",
      ...
      "claude-confirmation": {
        ...
        handler: (match, sessionId) => {
          ...
          console.log(
            `Claude confirmation option detected: ${number}. ${option}`,
          );
          return {
            state: "waiting",
            data: {
      ...
      "ai-success": {
        pattern:
          /✓\s+Successfully\s+(.+)|(?:Completed|Finished|Done):\s*(.+)|Human:/i,
        type: "completion",
        priority: "high",
        handler: (match, sessionId) => ({
          state: "idle",
      ...
      "ai-confused": {
        pattern:
          /I'm not sure|I need clarification|I don't understand|Could you clarify/i,
        type: "input",
        priority: "high",
        handler: (match, sessionId) => ({
          state: "waiting",
  ...
  setupGlobalMonitoring() {
    ...
    this.sessionManager.onData((sessionId, data, metadata) => {
      try {
        // Validate inputs
        if (!sessionId || !data) {
          console.warn("Invalid onData call:", {
            sessionId,
        ...
        if (data.length > 0) {
          console.log(
            `[AIMonitor] Processing data for ${sessionId}: ${data.length} bytes, first 50 chars: "${data.substring(0, 50).replace(/\n/g, "\\n")}"`,
          );
        }
        ...
        if (foundThinking) {
          ...
          this.patternMatchers.forEach((config, patternName) => {
            ...
            if (
              patternName === "claude-thinking" ||
              patternName === "claude-processing"
            ) {
              return;
            }
            ...
            if (match) {
              console.log(
                `Pattern matched: ${patternName} in session ${sessionId}`,
              );
        ...
        if (foundBashPrompt) {
          ...
          console.log(
            `Bash prompt found for ${sessionId}, current state: ${tracker?.currentState}`,
          );
          if (tracker && tracker.currentState !== "not-started") {
            console.log(
              "Bash prompt detected - Claude exited, changing state from",
        ...
        if (foundConfirmation) {
          ...
          if (!foundThinking && tracker.currentState !== "waiting") {
            console.log(
              "Multi-line input detected - transitioning to WAITING_INPUT",
            );
  ...
  async registerSessionPatterns(sessionId) {
    ...
    console.log(
      `Session ${sessionId} registered - patterns monitored via onData`,
    );
  }
  ...
  async broadcastStateUpdate(sessionId, status) {
    ...
    if (this.models && this.models.sessions && dbSessionId) {
      try {
        await this.models.sessions.updateSessionAIState(
          dbSessionId,
          status.currentState,
        );
      } catch (error) {
        console.error(
          `Failed to update AI state for session ${dbSessionId}:`,
          error,
        );
      }
    }
    ...
    if (this.models && this.models.sessions) {
      try {
        const activeSessions =
          await this.models.sessions.findAllActiveByTaskId(taskId);
        ...
        console.error(
          `Failed to get active sessions for task ${taskId}:`,
          error,
        );
      }
    }
    ...
    if (this.eventEmitterService) {
      this.eventEmitterService.emit(AI_EVENTS.STATE_CHANGED, {
        taskId,
        sessionState: {
    ...
    console.log(
      `AI state update: session ${sessionId} -> ${status.currentState}, task ${taskId} aggregate -> ${aggregateState}`,
    );
  }
