...
- `<type>`: The type of guide to create
  ...
  - `migration` - Complex system migration guide (like codebase separation)
...
1. **Pre-Flight Summary**
   ...
   - Example:
     ...
     - Estimated time: [realistic range, e.g., ~20-30 minutes]
     - Known risks: [top 3-5 risks]
     - Key dependencies: [critical requirements]
     ```

2. **Pre-execution Environment Check**
   - Exact commands to verify prerequisites
   - Clear checklist format
   - State tracking initialization

3. **Tool Usage Patterns**
   - TodoWrite structure with exact steps (create immediately)
   ...
   - Example:
     ...
     Execute this [task] using these specific tool patterns:
...
const guideState = {
  ...
  CRITICAL_CHECKS_DONE: false,
  TESTS_PASSED: false,
  
  // Data Collection
  ERRORS_ENCOUNTERED: [],    // {step: N, error: "message"}
  WARNINGS: [],
  CRITICAL_PATHS: [],        // Paths/configs that need special attention
  ...
  VERIFICATION_STATUS: {}    // Component-specific checks
};
\`\`\`
